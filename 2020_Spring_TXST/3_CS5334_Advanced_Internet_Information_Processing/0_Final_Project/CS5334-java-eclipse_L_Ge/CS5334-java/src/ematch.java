...
//
//           02/22/2012, Oracle 11g
//           03/01/2012, use of JDBC thin driver
//           03/11/2013
//           02/09/2015
//           03/27/2017
//
...
public class ematch extends HttpServlet {
    ...
    //  
    public String askedJobTitle;    // job title selected for search 	
	...
	/* comment out to  make job title a rating  
	if ( !paraValues[constStruct.JOB_TABLE_JOB_TITLE].equalsIgnoreCase("all") )
	{
	   if (addAND == 1)
	      jobQuerySqlStmt += " AND job_title='" + paraValues[constStruct.JOB_TABLE_JOB_TITLE] + "'";
	   else
	      jobQuerySqlStmt += " where job_title='" + paraValues[constStruct.JOB_TABLE_JOB_TITLE] + "'";
	   addAND = 1;
	}
	*/
	paraValues[constStruct.JOB_TABLE_SPECIALIZATION] = request.getParameter("specialty");
	
	/* comment out to make specification a rating.
	if ( !paraValues[constStruct.JOB_TABLE_SPECIALIZATION].equalsIgnoreCase("all") )
	{
	   if (addAND == 1)
	      jobQuerySqlStmt += " AND specialization='" +  paraValues[constStruct.JOB_TABLE_SPECIALIZATION] + "'";
	   else {
	...
	*/ 
	...
	// keyword seach term saved in description 
	...
	Connection conn = DriverManager.getConnection
	     ("jdbc:oracle:oci:@orcl", constStruct.USERNAME, constStruct.PASSWORD);
	...
	//out.println("<body background=\"/l_w152/images/sky_bg.gif\"" +
	...
	out.println("<A HREF=\"mailto:" + oneColumn + "\">" + oneColumn +
			"</A> <IMG SRC=/l_w152/images/email.gif border=0 align=middle>");
	out.println("</font>");
	out.println("<br>");
...
//Connection conn = ds.getConnection();
 ...
 DriverManager.registerDriver(new oracle.jdbc.driver.OracleDriver());
    ...
    Connection conn =
        DriverManager.getConnection ("jdbc:oracle:thin:@147.26.231.157:1521:"+ constStruct.DATABASE,
...
//      conn = ods.getConnection();
	...
	out.println("<html>");
	out.println("<head>");
	// this script is used to hide/show the detail search log
	out.println("<script language=\"JavaScript\" type=\"text/JavaScript\">");
	out.println("function toggle(targetid){"); 
	out.println("    if (document.getElementById){");
	out.println("	        target=document.getElementById(targetid);");
	out.println("            if (target.style.display==\"block\"){");
	out.println("	                target.style.display=\"none\";");
	out.println("	            } else {   ");
	out.println("	                target.style.display=\"block\"; ");
	out.println("	            } ");
	out.println("	} ");
	out.println("}");
	out.println("</script> ");
        ...
        throws IOException, ServletException
    {
		...
		//out.println("<td><a href=\"/l_w152/servlets/getjob?id=$count&filename=dummy\"" +
		//		"target=display>" + resultRows[k][3] + "</a>"); // job title
		out.println("<td><a href=\"/l_w152/servlet/getJob?job_id=" + 
				resultRows[k][1] + "\"" +
				"target=display>" + resultRows[k][3] + "</a>"); // job title
		...
		// description
		String desc = resultRows[k][constStruct.JOB_TABLE_DESCRIPTION + 1];
		if(desc == null || desc.equalsIgnoreCase("null")) {
			desc = "&nbsp;";
		}
		out.println("<td>" + desc);  // description
	    }
	    out.println("</tr>");
        ...
        throws IOException, ServletException
    {
        ...
        out.println("	<td> <A href=\"/l_w152/servlets/employerLogin.html\">Employers </A>");
        out.println("	<td> <A href=\"/l_w152/servlets/candidateLogin.html\">Members </A>");
        out.println("</tr>");
        out.println("</table>");
        out.println("</center>");
	...
	for (i = 0; i < constStruct.COLUMN_NUMBER_IN_JOB_TABLE; i++)
	{
		if ((paraValues[i] != null) && !paraValues[i].equals("All") &&
			...
			/* Search by region */
			if ((i==constStruct.JOB_TABLE_REGION_NAME ) &&
						(locationSearchType==constStruct.SEARCH_BY_REGION))
			{
				tmpRating -= computeRegionRating(curRow, out);
			} /* Search by state */
			else if ((i==constStruct.JOB_TABLE_STATE_NAME ) &&
					(locationSearchType==constStruct.SEARCH_BY_STATE))
			{
				tmpRating -= computeStateRating(askedLocationIndex, curRow, out);
			} /* Search by city */
			else if ((i==constStruct.JOB_TABLE_LOCATION) &&
					(locationSearchType==constStruct.SEARCH_BY_CITY))
			{
				tmpRating -= computeCityRating(askedLocationIndex, curRow, out);
			} // salary
			else  if ((i==constStruct.JOB_TABLE_MIN_SALARY) && (askedMinSal!= 0) &&
							(curRow[i+1] != null))
			{
			...
			// job_title_rating
			else if ((i==constStruct.JOB_TABLE_JOB_TITLE) && !paraValues[i].equalsIgnoreCase("all") ) 
			{
				out.println("Check job_title for rating " + i  );
				// paraValues[constStruct.JOB_TABLE_JOB_TITLE] is user selected job title
				// curRow[constStruct.JOB_TABLE_JOB_TITLE + 1]  is the job title of current row
				tmpRating -= computeJobTitleRating(paraValues[constStruct.JOB_TABLE_JOB_TITLE], curRow[constStruct.JOB_TABLE_JOB_TITLE + 1], out);
			}
			// specification rating
			else if ((i== constStruct.JOB_TABLE_SPECIALIZATION) && !paraValues[i].equalsIgnoreCase("all") ) 
			{
				out.println("Check specification for rating " + i  );
				// paraValues[constStruct.JOB_TABLE_SPECIALIZATION] is user selected specification
				// curRow[constStruct.JOB_TABLE_SPECIALIZATION + 1]  is the specification of current row
				tmpRating -= computeSpecificationRating(paraValues[constStruct.JOB_TABLE_SPECIALIZATION], curRow[constStruct.JOB_TABLE_SPECIALIZATION + 1], out);
			} 
			// keyword/description rating
			else if ((i== constStruct.JOB_TABLE_DESCRIPTION) && paraValues[constStruct.JOB_TABLE_DESCRIPTION].length() > 0) {
				out.println("Check keyword/specification for rating " + i + " keyowrd:"+ paraValues[constStruct.JOB_TABLE_DESCRIPTION]);
    ...
    */
    ...
    /**
     * this is to find the index of name in the list
     * 
     * @param list
     * @param name
     * @return   first index of name in the list or -1 if not found, 
     */
    
    int findIndex(String [] list, String name)   {
    	if(list == null  || list.length == 0)
    		return -1;
    	for (int i = 0; i < list.length; i++) {
    		if (name.compareToIgnoreCase(list[i]) == 0)
    			return i;
    	}
    	return -1;
    ...
    /**
     * just look up the static table to 
     * compute the title penalty 
     * 
     * @param selectedTitle: selected job title
     * @param currentTitle:  target job title
     * @param out
     * @return: 			penalty of current job 
     * 
     * @throws IOException
     * @throws ServletException
     */
    public int computeJobTitleRating (String selectedTitle, String currentTitle, PrintWriter out) 
    		throws IOException, ServletException    {
    	out.println("ematch.computeJobTitleRating() select: "+ selectedTitle + " current:"+ currentTitle );
    	int selectedIndex = findIndex (this.constStruct.jobTitleList, selectedTitle);
    	int currentIndex = findIndex(this.constStruct.jobTitleList, currentTitle);
    	if(selectedIndex == -1  || currentIndex == -1)
    		return 100; 
    	// look up in the table
    	int rating = constStruct.jobTitleRatingList[selectedIndex][currentIndex];
    	out.println("  selectIndex:"+ selectedIndex + " currentIndex: "+ currentIndex  + " rating:"+rating+ " <br>");
    	return rating; 
    }     
    /**
     * compute the specification rating 
     * using dynamic logic.
     * 
     * 	For example, if the user selects "AAA BBB" as specification for search,  
	 *	First, I split it to two phrases, AAA and BBB, than check if target specification contains each phrase, 
	 ...
	 *	If the selected specification is "AAA BBB CCC", and target is "ZZZ BBB YYY", which contains 2 phrase, the penalty would be 67 (100 * (1-1/3)) 
	 *	As summary, the penalty is  100 * (1 - number_of_matched_phrases/number_of_total_phrases) 
     * 
     * @param selectedSpec		selected specification
     * @param currentSpec		target specification in current row
     * @param out				output stream for debuging
     * @return					penalty for the target row
     * 
     * @throws IOException
     * @throws ServletException
     */
    public int computeSpecificationRating (String selectedSpec, String currentSpec, PrintWriter out) 
    		throws IOException, ServletException    {
    	out.println("ematch.computeSpecificationRating() select: "+ selectedSpec + " current:"+ currentSpec );
    	if(currentSpec == null || currentSpec.length() == 0) // current row has not specification, drop it. 
    		return 100;
    	// split the selected
    	String [] specs = selectedSpec.split(" ");
    	int total = specs.length;  // total number of phrases
    	int matched = 0;    // matched number 
    	for(String one : specs) {
    		if(currentSpec.contains(one)) {
    			matched ++;
    		}
    	}
    	out.println(", total specs:"+ total +" matched specs:"+matched);
    	int rating = (int) (100 * ( 1.0 - (float) matched/ (float) total));
    	out.println(", rating: "+ rating + " <br>");
    	return rating; 
    }     
    /**
     * compute the penalty for keyword
     * 
     * @param keyword, selected keyword
     * @param oneRow, target result row
     * @param out, for output/debug
     * @return, the penalty of target result row
     * 
     * @throws IOException
     * @throws ServletException
     */
    public int computeKeywordRating (String keyword, String oneRow[], PrintWriter out)  throws IOException, ServletException {
    	...
    	out.println("computeKeywordRating() " + keyword + " " + oneRow[constStruct.JOB_TABLE_DESCRIPTION + 1] + " " + oneRow[constStruct.JOB_TABLE_SPECIALIZATION + 1] );
    	// if description is not empty and contains the keyword
    	if(  (oneRow[constStruct.JOB_TABLE_DESCRIPTION + 1] != null) &&
    		...
    		oneRow[constStruct.JOB_TABLE_DESCRIPTION + 1] = 
    				oneRow[constStruct.JOB_TABLE_DESCRIPTION + 1].replaceAll("(?i)"+keyword, "<font color=\"red\"><b>"+keyword+"</b></font>");
    		foundInDescription = true;
    	} 
    	// if specification is not empty and contains the keyword
    	if( (oneRow[constStruct.JOB_TABLE_SPECIALIZATION + 1] != null) &&
    		oneRow[constStruct.JOB_TABLE_SPECIALIZATION + 1].toLowerCase().contains(keyword.toLowerCase())) {
    		// highlight each  keyword
    		oneRow[constStruct.JOB_TABLE_SPECIALIZATION + 1] =
    				oneRow[constStruct.JOB_TABLE_SPECIALIZATION + 1].replaceAll("(?i)"+keyword, "<font color=\"red\"><b>"+keyword+"</b></font>");
    		foundInSpec = true;
    	}
    	int rating = 100;
    	if(foundInDescription) { // main search field, no penalty 
    		rating = 0;
    	} else if(foundInSpec) {// second field with penaly
    		rating = 50;
    	}	
    	out.println("<br>computeKeywordRating()" + keyword +  " descrition:" + oneRow[constStruct.JOB_TABLE_DESCRIPTION + 1] +" spec:" +oneRow[constStruct.JOB_TABLE_SPECIALIZATION + 1]  +" rating:" + rating);
    	return rating; 
    }
}
