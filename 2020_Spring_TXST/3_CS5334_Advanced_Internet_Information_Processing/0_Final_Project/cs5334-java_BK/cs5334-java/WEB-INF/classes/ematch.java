...
public class ematch extends HttpServlet {
	...
	public ematch(int ematchOrInsert) {
		...
		if (ematchOrInsert == constStruct.EMATCH_) {
			...
			for (i = 1; i < constStruct.NUMBER_OF_US_CITIES; i++) {
				neighborCityBeginningIndex[i] = neighborCityBeginningIndex[i - 1] + 2 * constStruct.usNeighborCityNumber[i - 1];
			}

			for (i = 1; i < constStruct.NUMBER_OF_US_STATES; i++) {
				usCityInStateBeginningIndex[i] += usCityInStateBeginningIndex[i - 1] + constStruct.usCitiesInStatesNumber[i - 1];
			}

			for (i = 1; i < constStruct.NUMBER_OF_US_STATES; i++) {

				neighborStateBeginningIndex[i] = neighborStateBeginningIndex[i - 1] + 2 * constStruct.usNeighborStateNumber[i - 1];
			}

			for (i = 1; i < constStruct.NUMBER_OF_US_REGIONS; i++) {
				usStateInRegionBeginningIndex[i] = usStateInRegionBeginningIndex[i - 1] + constStruct.usStatesInRegionsNumber[i - 1] + 1;
			}
	...
	IOException {
		...
		if (!paraValues[constStruct.JOB_TABLE_JOB_TITLE].equalsIgnoreCase("all")) {
			if (addAND == 1) jobQuerySqlStmt += " AND job_title='" + paraValues[constStruct.JOB_TABLE_JOB_TITLE] + "'";
			else jobQuerySqlStmt += " where job_title='" + paraValues[constStruct.JOB_TABLE_JOB_TITLE] + "'";
			addAND = 1;
		}

		paraValues[constStruct.JOB_TABLE_SPECIALIZATION] = request.getParameter("specialty");

		if (!paraValues[constStruct.JOB_TABLE_SPECIALIZATION].equalsIgnoreCase("all")) {
			if (addAND == 1) jobQuerySqlStmt += " AND specialization='" + paraValues[constStruct.JOB_TABLE_SPECIALIZATION] + "'";
			else {
				jobQuerySqlStmt += " where specialization='" + paraValues[constStruct.JOB_TABLE_SPECIALIZATION] + "'";
				addAND = 1;
			}
		}
		...
		if (!paraValues[constStruct.JOB_TABLE_COMPANY_NAME].equalsIgnoreCase("any")) {
			if (addAND == 1) jobQuerySqlStmt += " AND company_name='" + paraValues[constStruct.JOB_TABLE_COMPANY_NAME] + "'";
			else {
	...
	IOException {
		...
		if (!paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_TITLE].equalsIgnoreCase("all")) {
			if (addAND == 1) candidateQuerySqlStmt += " AND job_title='" + paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_TITLE] + "'";
			else candidateQuerySqlStmt += " where job_title='" + paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_TITLE] + "'";
			addAND = 1;
		}
		...
		if (!paraValues[constStruct.MEMBER_TABLE_SPECIALIZATION].equalsIgnoreCase("all")) {
			if (addAND == 1) candidateQuerySqlStmt += " AND specialization='" + paraValues[constStruct.MEMBER_TABLE_SPECIALIZATION] + "'";
			else {
				candidateQuerySqlStmt += " where specialization='" + paraValues[constStruct.MEMBER_TABLE_SPECIALIZATION] + "'";
				addAND = 1;
			}
		}
	...
	IOException {
		...
		int jobTypeMissing = 0,
		jobTitleMissing,
		specializationMissing = 0;
		int regionMissing = 0,
		stateMissing = 0,
		cityMissing = 0;
		int companyNameMissing = 0,
		startDateMissing = 0;
		...
		if (paraValues[constStruct.JOB_TABLE_JOB_TYPE] != null) {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_JOB_TYPE] + paraValues[constStruct.JOB_TABLE_JOB_TYPE].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_JOB_TYPE] + ", ";
		} else {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_JOB_TYPE] + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_JOB_TYPE] + ", ";
			numberOfMissingFields++;
		}

		paraValues[constStruct.JOB_TABLE_JOB_TITLE] = request.getParameter("jobTitle");
		if (!paraValues[constStruct.JOB_TABLE_JOB_TITLE].equals("null")) {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_JOB_TITLE] + paraValues[constStruct.JOB_TABLE_JOB_TITLE].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_JOB_TITLE] + ", ";
		} else {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_JOB_TITLE] + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_JOB_TITLE] + ", ";
			numberOfMissingFields++;
		}

		paraValues[constStruct.JOB_TABLE_SPECIALIZATION] = request.getParameter("specialty");
		if (!paraValues[constStruct.JOB_TABLE_SPECIALIZATION].equals("null")) {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_SPECIALIZATION] + paraValues[constStruct.JOB_TABLE_SPECIALIZATION].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_SPECIALIZATION] + ", ";
		} else {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_SPECIALIZATION] + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_SPECIALIZATION] + ", ";
			numberOfMissingFields++;
		}
		...
		//     } else { 
		insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_COMPANY_NAME] + "1" + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_COMPANY_NAME] + ", ";
		//    }
		paraValues[constStruct.JOB_TABLE_REGION_NAME] = request.getParameter("regionName");
		if (paraValues[constStruct.JOB_TABLE_REGION_NAME].equals("Any") || paraValues[constStruct.JOB_TABLE_REGION_NAME].equals("null")) {
			regionMissing = 1;
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_REGION_NAME] + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_REGION_NAME] + ", ";
		} else if (!paraValues[constStruct.JOB_TABLE_REGION_NAME].equals("null")) {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_REGION_NAME] + paraValues[constStruct.JOB_TABLE_REGION_NAME].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_REGION_NAME] + ", ";
		}
		...
		if (paraValues[constStruct.JOB_TABLE_STATE_NAME].equals("Any")) {
			stateMissing = 1;
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_STATE_NAME] + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_STATE_NAME] + ", ";
		}
		else if (!paraValues[constStruct.JOB_TABLE_STATE_NAME].equals("null")) {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_STATE_NAME] + paraValues[constStruct.JOB_TABLE_STATE_NAME].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_STATE_NAME] + ", ";
		}
		...
		if (paraValues[constStruct.JOB_TABLE_LOCATION].equals("Any")) {
			cityMissing = 1;
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_LOCATION] + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_LOCATION] + ", ";
		} else if (!paraValues[constStruct.JOB_TABLE_LOCATION].equals("null")) {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_LOCATION] + paraValues[constStruct.JOB_TABLE_LOCATION].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_LOCATION] + ", ";
		}
		...
		if (!paraValues[constStruct.JOB_TABLE_MIN_SALARY].equals("null")) {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_MIN_SALARY] + paraValues[constStruct.JOB_TABLE_MIN_SALARY].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_MIN_SALARY] + ", ";
		} else { // set min salary to 0 if it is not supplied
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_MIN_SALARY] + 0 + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_MIN_SALARY] + ", ";
		}
		...
		if (!paraValues[constStruct.JOB_TABLE_MAX_SALARY].trim().equals("")) {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_MAX_SALARY] + paraValues[constStruct.JOB_TABLE_MAX_SALARY].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_MAX_SALARY] + ", ";
		} else {
			if (paraValues[constStruct.JOB_TABLE_MIN_SALARY].equals("")) insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_MAX_SALARY] + 0 + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_MAX_SALARY] + ", ";
			else // otherwise, set max salary same as min salary
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_MAX_SALARY] + paraValues[constStruct.JOB_TABLE_MIN_SALARY].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_MAX_SALARY] + ", ";
		}
		...
		if (paraValues[constStruct.JOB_TABLE_COMPANY_NAME].equals("null")) {
			...
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_COMPANY_NAME] + paraValues[constStruct.JOB_TABLE_COMPANY_NAME].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_COMPANY_NAME] + ", ";
		}
		...
		if (paraValues[constStruct.JOB_TABLE_START_DATE].equals("null")) {
			...
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_START_DATE] + paraValues[constStruct.JOB_TABLE_START_DATE].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_START_DATE] + ", ";
		}
		...
		if (!paraValues[constStruct.JOB_TABLE_REFERENCE_NUM].equals("null")) {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_REFERENCE_NUM] + paraValues[constStruct.JOB_TABLE_REFERENCE_NUM].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_REFERENCE_NUM] + ", ";
		} else {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_REFERENCE_NUM] + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_REFERENCE_NUM] + ", ";
		}
		...
		if (!paraValues[constStruct.JOB_TABLE_CONTACT_PERSON].equals("null")) {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_CONTACT_PERSON] + paraValues[constStruct.JOB_TABLE_CONTACT_PERSON].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_CONTACT_PERSON] + ", ";
		} else {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_CONTACT_PERSON] + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_CONTACT_PERSON] + ", ";
		}
		...
		if (paraValues[constStruct.JOB_TABLE_DESCRIPTION] != null) {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_DESCRIPTION] + paraValues[constStruct.JOB_TABLE_DESCRIPTION].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_DESCRIPTION] + ", ";
		} else {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_DESCRIPTION] + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_DESCRIPTION] + ", ";
		}
		...
		if (paraValues[constStruct.JOB_TABLE_QUALIFICATION] != null) {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_QUALIFICATION] + paraValues[constStruct.JOB_TABLE_QUALIFICATION].trim() + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_QUALIFICATION] + ")";
		} else {
			insertJobStmt += constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_QUALIFICATION] + constStruct.jobTableColumnQuotes[constStruct.JOB_TABLE_QUALIFICATION] + ")";
		}

		if ((regionMissing == 1) && (stateMissing == 1) && (cityMissing == 1)) return "Error: at least one of region, state, city names should be given: " + insertJobStmt;
		else if (numberOfMissingFields > 0) return "Error: total " + numberOfMissingFields + " number of required fields missing:" + insertJobStmt;
	...
	IOException {
		...
		int empIdMissing = 0,
		companyNameMissing = 0,
		contactPersonMissing = 0;
		int emailMissing,
		passwordMissing = 0;
		...
		if ((paraValues[constStruct.EMPLOYER_TABLE_EMP_ID] == null) || (paraValues[constStruct.EMPLOYER_TABLE_EMP_ID].equals(""))) return "Error: Missing employer Id";
		else if (!idIsUnique(paraValues[constStruct.EMPLOYER_TABLE_EMP_ID], constStruct.EMPLOYER_TABLE_INDEX)) return "Error: selected employer Id is already being used. Please choose a different one";
		insertEmpStmt += constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_EMP_ID] + paraValues[constStruct.EMPLOYER_TABLE_EMP_ID] + constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_EMP_ID] + ", ";

		paraValues[constStruct.EMPLOYER_TABLE_COMPANY_NAME] = String.valueOf(request.getParameter("companyName")).trim();
		if ((paraValues[constStruct.EMPLOYER_TABLE_COMPANY_NAME] == null) || (paraValues[constStruct.EMPLOYER_TABLE_COMPANY_NAME].equals(""))) return "Error: Missing company name";
		insertEmpStmt += constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_COMPANY_NAME] + paraValues[constStruct.EMPLOYER_TABLE_COMPANY_NAME] + constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_COMPANY_NAME] + ", ";

		paraValues[constStruct.EMPLOYER_TABLE_CONTACT_PERSON] = String.valueOf(request.getParameter("contactPerson")).trim();
		if ((paraValues[constStruct.EMPLOYER_TABLE_CONTACT_PERSON] == null) || (paraValues[constStruct.EMPLOYER_TABLE_CONTACT_PERSON].equals(""))) return "Error: Missing contact person name";
		insertEmpStmt += constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_CONTACT_PERSON] + paraValues[constStruct.EMPLOYER_TABLE_CONTACT_PERSON] + constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_CONTACT_PERSON] + ", ";

		paraValues[constStruct.EMPLOYER_TABLE_EMAIL] = String.valueOf(request.getParameter("email")).trim();
		if ((paraValues[constStruct.EMPLOYER_TABLE_EMAIL] == null) || (paraValues[constStruct.EMPLOYER_TABLE_EMAIL].equals(""))) return "Error: Missing contact person name";
		insertEmpStmt += constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_EMAIL] + paraValues[constStruct.EMPLOYER_TABLE_EMAIL] + constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_EMAIL] + ", ";

		paraValues[constStruct.EMPLOYER_TABLE_PHONE] = String.valueOf(request.getParameter("phone")).trim();
		if (paraValues[constStruct.EMPLOYER_TABLE_PHONE] == null) insertEmpStmt += constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_PHONE] + constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_PHONE] + ", ";
		else insertEmpStmt += constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_PHONE] + paraValues[constStruct.EMPLOYER_TABLE_PHONE] + constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_PHONE] + ", ";

		paraValues[constStruct.EMPLOYER_TABLE_FAX] = String.valueOf(request.getParameter("fax")).trim();
		if (paraValues[constStruct.EMPLOYER_TABLE_FAX] == null) insertEmpStmt += constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_FAX] + constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_FAX] + ", ";
		else insertEmpStmt += constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_FAX] + paraValues[constStruct.EMPLOYER_TABLE_FAX] + constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_FAX] + ", ";

		paraValues[constStruct.EMPLOYER_TABLE_PROFILE] = String.valueOf(request.getParameter("profile")).trim();
		if (paraValues[constStruct.EMPLOYER_TABLE_PROFILE] == null) insertEmpStmt += constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_PROFILE] + constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_PROFILE] + ", ";
		else insertEmpStmt += constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_PROFILE] + paraValues[constStruct.EMPLOYER_TABLE_PROFILE] + constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_PROFILE] + ", ";

		paraValues[constStruct.EMPLOYER_TABLE_PASSWORD] = String.valueOf(request.getParameter("password")).trim();
		if ((paraValues[constStruct.EMPLOYER_TABLE_PASSWORD] == null) || (paraValues[constStruct.EMPLOYER_TABLE_PASSWORD].equals(""))) return "Error: Missing password";
		else if (paraValues[constStruct.EMPLOYER_TABLE_PASSWORD].length() <= 5) return "Error: Password should be less than six characters";
		insertEmpStmt += constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_PASSWORD] + paraValues[constStruct.EMPLOYER_TABLE_PASSWORD] + constStruct.employerTableColumnQuotes[constStruct.EMPLOYER_TABLE_PASSWORD] + ") ";
	...
	IOException {
		...
		if ((paraValues[constStruct.MEMBER_TABLE_LOGIN_ID] == null) || (paraValues[constStruct.MEMBER_TABLE_LOGIN_ID].equals(""))) return "Error: Missing login Id";
		else if (!idIsUnique(paraValues[constStruct.MEMBER_TABLE_LOGIN_ID], constStruct.MEMBER_TABLE_INDEX)) return "Error: selected login Id is already being used. Please choose a different one";
		insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_LOGIN_ID] + paraValues[constStruct.MEMBER_TABLE_LOGIN_ID] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_LOGIN_ID] + ", ";

		paraValues[constStruct.MEMBER_TABLE_FIRST_NAME] = String.valueOf(request.getParameter("firstName")).trim();
		if ((paraValues[constStruct.MEMBER_TABLE_FIRST_NAME] == null) || (paraValues[constStruct.MEMBER_TABLE_FIRST_NAME].equals(""))) return "Error: Missing first name";
		insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_FIRST_NAME] + paraValues[constStruct.MEMBER_TABLE_FIRST_NAME] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_FIRST_NAME] + ", ";

		paraValues[constStruct.MEMBER_TABLE_MID_I_NAME] = String.valueOf(request.getParameter("midIName")).trim();
		if ((paraValues[constStruct.MEMBER_TABLE_MID_I_NAME].equals("null")) || (paraValues[constStruct.MEMBER_TABLE_MID_I_NAME].equals(""))) insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_MID_I_NAME] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_MID_I_NAME] + ", ";
		else insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_MID_I_NAME] + paraValues[constStruct.MEMBER_TABLE_MID_I_NAME] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_MID_I_NAME] + ", ";

		paraValues[constStruct.MEMBER_TABLE_LAST_NAME] = String.valueOf(request.getParameter("lastName")).trim();
		if ((paraValues[constStruct.MEMBER_TABLE_LAST_NAME].equals("null")) || (paraValues[constStruct.MEMBER_TABLE_LAST_NAME].equals(""))) return "Error: Missing last name";
		insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_LAST_NAME] + paraValues[constStruct.MEMBER_TABLE_LAST_NAME] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_LAST_NAME] + ", ";

		paraValues[constStruct.MEMBER_TABLE_SPECIALIZATION] = request.getParameter("specialty");
		if (paraValues[constStruct.MEMBER_TABLE_SPECIALIZATION].equals("null")) {
			return "Error: Missing specialty";
		} else {
			insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_SPECIALIZATION] + paraValues[constStruct.MEMBER_TABLE_SPECIALIZATION] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_SPECIALIZATION] + ", ";
		}

		paraValues[constStruct.MEMBER_TABLE_EMAIL] = String.valueOf(request.getParameter("email")).trim();
		if ((paraValues[constStruct.MEMBER_TABLE_EMAIL].equals("null")) || (paraValues[constStruct.MEMBER_TABLE_EMAIL].equals(""))) return "Error: Missing email";
		insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_EMAIL] + paraValues[constStruct.MEMBER_TABLE_EMAIL] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_EMAIL] + ", ";

		paraValues[constStruct.MEMBER_TABLE_PHONE] = String.valueOf(request.getParameter("phone")).trim();
		if (paraValues[constStruct.MEMBER_TABLE_PHONE].equals("null")) insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_PHONE] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_PHONE] + ", ";
		else insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_PHONE] + paraValues[constStruct.MEMBER_TABLE_PHONE] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_PHONE] + ", ";

		paraValues[constStruct.MEMBER_TABLE_FAX] = String.valueOf(request.getParameter("fax")).trim();
		if (paraValues[constStruct.MEMBER_TABLE_FAX].equals("null")) insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_FAX] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_FAX] + ", ";
		else insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_FAX] + paraValues[constStruct.MEMBER_TABLE_FAX] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_FAX] + ", ";

		paraValues[constStruct.MEMBER_TABLE_WEB_URL] = String.valueOf(request.getParameter("webURL")).trim();
		if (paraValues[constStruct.MEMBER_TABLE_WEB_URL].equals("null")) insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_WEB_URL] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_WEB_URL] + ", ";
		else insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_WEB_URL] + paraValues[constStruct.MEMBER_TABLE_WEB_URL] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_WEB_URL] + ", ";

		paraValues[constStruct.MEMBER_TABLE_CURRENT_COMPANY] = String.valueOf(request.getParameter("currentCompany")).trim();
		if (paraValues[constStruct.MEMBER_TABLE_CURRENT_COMPANY].equals("null")) insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_CURRENT_COMPANY] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_CURRENT_COMPANY] + ", ";
		else insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_CURRENT_COMPANY] + paraValues[constStruct.MEMBER_TABLE_CURRENT_COMPANY] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_CURRENT_COMPANY] + ", ";

		paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_TITLE] = String.valueOf(request.getParameter("currentJobTitle")).trim();
		if ((paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_TITLE].equals("null")) || (paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_TITLE].equals(""))) return "Error: Missing current job title";
		insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_CURRENT_JOB_TITLE] + paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_TITLE] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_CURRENT_JOB_TITLE] + ", ";
		...
		if (locationType.equals("region")) {
			...
			paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_LOCATION_CODE] = Integer.toString(constStruct.SEARCH_BY_REGION);
		} else if (locationType.equals("state")) {
			paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_LOCATION] = request.getParameter("currentState");
			paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_LOCATION_CODE] = Integer.toString(constStruct.SEARCH_BY_STATE);
		} else if (locationType.equals("city")) {
			paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_LOCATION] = request.getParameter("currentCity");
			paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_LOCATION_CODE] = Integer.toString(constStruct.SEARCH_BY_CITY);
		}

		insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_CURRENT_JOB_LOCATION] + paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_LOCATION] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_CURRENT_JOB_LOCATION] + ", ";

		insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_CURRENT_JOB_LOCATION_CODE] + paraValues[constStruct.MEMBER_TABLE_CURRENT_JOB_LOCATION_CODE] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_CURRENT_JOB_LOCATION_CODE] + ", ";

		paraValues[constStruct.MEMBER_TABLE_YEAR_OF_EXP] = String.valueOf(request.getParameter("yearOfExp")).trim();
		if ((paraValues[constStruct.MEMBER_TABLE_YEAR_OF_EXP].equals("null")) || (paraValues[constStruct.MEMBER_TABLE_YEAR_OF_EXP].equals(""))) return "Error: Missing years of experience";
		insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_YEAR_OF_EXP] + paraValues[constStruct.MEMBER_TABLE_YEAR_OF_EXP] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_YEAR_OF_EXP] + ", ";

		paraValues[constStruct.MEMBER_TABLE_DEGREE] = String.valueOf(request.getParameter("degree")).trim();
		if ((paraValues[constStruct.MEMBER_TABLE_DEGREE].equals("null")) || (paraValues[constStruct.MEMBER_TABLE_DEGREE].equals(""))) return "Error: Missing degree";
		insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DEGREE] + paraValues[constStruct.MEMBER_TABLE_DEGREE] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DEGREE] + ", ";

		paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_1] = String.valueOf(request.getParameter("desiredJob1")).trim();
		if (paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_1].equals("null")) insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_1] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_1] + ", ";
		else insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_1] + paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_1] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_1] + ", ";

		paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_2] = String.valueOf(request.getParameter("desiredJob2")).trim();
		if (paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_2].equals("null")) insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_2] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_2] + ", ";
		else insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_2] + paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_2] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_2] + ", ";

		paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_3] = String.valueOf(request.getParameter("desiredJob3")).trim();
		if (paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_3].equals("null")) insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_3] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_3] + ", ";
		else insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_3] + paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_3] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_3] + ", ";

		paraValues[constStruct.MEMBER_TABLE_DESIRED_SALARY] = String.valueOf(request.getParameter("desiredSalary")).trim();
		if (paraValues[constStruct.MEMBER_TABLE_DESIRED_SALARY] == null) insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_SALARY] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_SALARY] + ", ";
		else insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_SALARY] + paraValues[constStruct.MEMBER_TABLE_DESIRED_SALARY] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_SALARY] + ", ";
		...
		if (locationType.equals("region")) {
			...
			paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION_CODE] = Integer.toString(constStruct.SEARCH_BY_REGION);
		} else if (locationType.equals("state")) {
			paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION] = request.getParameter("desiredState");
			paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION_CODE] = Integer.toString(constStruct.SEARCH_BY_STATE);
		} else if (locationType.equals("city")) {
			paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION] = request.getParameter("desiredCity");
			paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION_CODE] = Integer.toString(constStruct.SEARCH_BY_CITY);
		}

		//paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION] =
		//		String.valueOf(request.getParameter("desiredJobLocation")).trim();
		insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION] + paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION] + ", ";

		insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION_CODE] + paraValues[constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION_CODE] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION_CODE] + ", ";

		paraValues[constStruct.MEMBER_TABLE_SPECIAL_TALENTS] = String.valueOf(request.getParameter("specialTalents")).trim();
		if (paraValues[constStruct.MEMBER_TABLE_SPECIAL_TALENTS].equals("null")) insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_SPECIAL_TALENTS] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_SPECIAL_TALENTS] + ", ";
		else insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_SPECIAL_TALENTS] + paraValues[constStruct.MEMBER_TABLE_SPECIAL_TALENTS] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_SPECIAL_TALENTS] + ", ";

		paraValues[constStruct.MEMBER_TABLE_RESUME] = String.valueOf(request.getParameter("resume")).trim();
		if (paraValues[constStruct.MEMBER_TABLE_RESUME] == null) insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_RESUME] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_RESUME] + ")";
		else insertCandidateStmt += constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_RESUME] + paraValues[constStruct.MEMBER_TABLE_RESUME] + constStruct.memberTableColumnQuotes[constStruct.MEMBER_TABLE_RESUME] + ")";

		return insertCandidateStmt;
	} // End of buildInsertCandidateStmt()

	public void getAndDisplayAJobDetails(HttpServletRequest request, HttpServletResponse response, PrintWriter out)
	throws SQLException,
	IOException {
		String queryStmt;
		String jobId;
		...
		if (oneColumn != null) {
			...
			if (oneColumn != null) {
				...
				oneColumn = rset.getString(7);
				if (oneColumn != null) out.print(", " + oneColumn); // region
				out.print(")");
			}
			else {
				oneColumn = rset.getString(6);
				if (oneColumn != null) out.print(" (" + oneColumn + ")"); // state
			}
			out.println("");
		}
		else {
			...
			else out.println(rset.getString(6)); // region
		}
		out.println("</font>");
		out.println("<br>");
		...
		out.println("<font color=\"#900000\">");
		if (oneColumn == null) out.println("N/A--");
		else out.println(oneColumn + "--");
	...
	public void getAndDisplayACandidate(HttpServletRequest request, HttpServletResponse response, PrintWriter out)
	throws SQLException,
	IOException {
		String queryStmt;
		String loginId;
		...
		oneColumn = rset.getString(6);
		out.println("<font color=\"#900000\">");
		out.println("<A HREF=\"mailto:" + oneColumn + "\">" + oneColumn + "</A> <IMG SRC=/peng/images/email.gif border=0 align=middle>");
		out.println("</font>");
		out.println("<br>");
	...
	ServletException {
		...
		for (int i = 0; i < numberOfMatchedRows; i++) {
			...
			out.println("<td><a href=\"/servlet/getJob?job_id=" + resultRows[k][1] + "\"" + "target=display>" + resultRows[k][3] + "</a>"); // job title
			...
			if (resultRows[k][8] != null) {
				...
				if (resultRows[k][7] != null) {
					...
					if (resultRows[k][6] != null) out.print(", " + resultRows[k][6]); // region
					out.print(")");
				}
				else if (resultRows[k][6] != null) out.print(" (" + resultRows[k][6] + ")"); // state
				out.println("");
			}
			...
			else out.println("<td>" + resultRows[k][6]); // region
	...
	ServletException {
		...
		for (int i = 0; i < numberOfMatchedRows; i++) {
			...
			//out.println("<td>" + resultRows[k][1]);
			out.println("<td>" + rating[i]);
			out.println("<td><a href=\"/servlet/getCandidate?login_id=" + resultRows[k][1] + "\"" + "target=display>" + resultRows[k][5] + "</a>"); // specialty
			...
			if (resultRows[k][20] != null) out.print("<td>" + resultRows[k][20]); // desired_job_location
			else out.print("<td>" + "N/A");
		}
		out.println("</tr>");
		out.println("</table></center>");
	...
	ServletException {
		...
		for (i = 0; i < constStruct.COLUMN_NUMBER_IN_MEMBER_TABLE; i++) {
			if ((paraValues[i] != null) && !paraValues[i].equals("All") && !paraValues[i].equals("Any")) {
				//out.println("CONSIDER column i="+i+" paraValues[i]="+paraValues[i] + "---");
				if ((i == constStruct.MEMBER_TABLE_YEAR_OF_EXP) && (curRow[i + 1] != null) && (askedYearOfExp > 0)) {
					tmpRating -= computeYearOfExpRating(curRow[i + 1], out);
				}
				else if ((i == constStruct.MEMBER_TABLE_DEGREE) && (curRow[i + 1] != null) && (askedDegree > -1)) {
					tmpRating -= computeDegreeRating(curRow[i + 1], out);
					/* Search by region */
				} else if ((i == constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION_CODE) && (locationSearchType == constStruct.SEARCH_BY_REGION)) {
					tmpRating -= computeRegionRating(curRow, out);
					/* Search by state */
				} else if ((i == constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION_CODE) && (locationSearchType == constStruct.SEARCH_BY_STATE)) {
					tmpRating -= computeStateRating(askedLocationIndex, curRow, out);
					/* Search by city */
				} else if ((i == constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION) && (locationSearchType == constStruct.SEARCH_BY_CITY)) {
					tmpRating -= computeCityRating(askedLocationIndex, curRow, out);
					// salary
				} else if ((i == constStruct.MEMBER_TABLE_DESIRED_SALARY) && (askedMaxSal != 0) && (curRow[i + 1] != null)) {
					sal = Integer.parseInt(curRow[i + 1]);
					//out.println("Check salary for row " + i + " with sal = " + sal);
					tmpRating -= computeSalRating(sal);
				}
    ...
    */
	...
	ServletException {
		...
		if (searchType == constStruct.JOB_SEARCH) {
			...
			if (regionIndex != -1) {
				...
				if (constStruct.regionCompability[regionIndex][askedLocationIndex] == 1) deduction = 0;
					...
					deduction = 70;
					if (stateIndex != -1) tmpDeduction = checkIfStateInRegion(stateIndex, askedLocationIndex, out);
					else if (cityIndex != -1) tmpDeduction = checkIfCityInRegion(cityIndex, askedLocationIndex, out);
			...
			if (oneRow[constStruct.MEMBER_TABLE_DESIRED_JOB_LOCATION + 1] != null) {
				...
				if (locationCode == 2) {
					...
					else if (constStruct.regionCompability[locationIndex][askedLocationIndex] == 1) deduction = 0;
	...
	ServletException {
		...
		int r = usStateInRegionBeginningIndex[regionIndex],
		s,
		l,
		st;
    ...
    */
	...
	ServletException {
		...
		if (searchType == constStruct.JOB_SEARCH) {
			...
			if (cityIndex != -1) { // The row does not have a valid state, but has a valid city
				...
				if (stateIndex == askedStateIndex) deduction = 35;
				else deduction = 10 + findNeighborStateRating(stateIndex, askedStateIndex, out);

			} else if (regionIndex != -1) { // The row does not have a valid state or city,                                                //    but does have a valid region name
				askedStateIndex = findTheStateContainingTheCity(askedCityIndex);
				if (checkIfStateInRegion(askedStateIndex, regionIndex, out) == 0) deduction = 50;
			}
	...
	ServletException {
		...
		for (i = 0; i < 2 * k; i = i + 2) {
			out.println("State " + constStruct.usStates[constStruct.usNeighborStateRatingList[i + j]] + " with index " + constStruct.usNeighborStateRatingList[i + j] + "<br>");
	...
	ServletException {
		...
		if (diff > 0) {
			if (diff <= 1) deduction = 10;
			else if (diff <= 2) deduction = 20;
			else if (diff <= 3) deduction = 30;
			else if (diff <= 4) deduction = 40;
			else deduction = 50;
		}
		return deduction;
	}
	...
	ServletException {
		...
		if (diff > 0) {
			if (diff <= 1) deduction = 5;
			else if (diff <= 2) deduction = 10;
			else if (diff <= 3) deduction = 15;
			else if (diff <= 4) deduction = 20;
			else deduction = 25;
		}
		return deduction;
	}
	...
	int findRegionIndex(String a_name) {
		...
		if (a_name.compareTo(constStruct.usRegions[i]) == 0) return i;
		return - 1;
	...
	int findStateIndex(String a_name) {
		...
		if (a_name.compareTo(constStruct.usStates[i]) == 0) return i;
		return - 1;
	...
	int findCityIndex(String a_name) {
		...
		if (a_name.compareTo(constStruct.usCities[i]) == 0) return i;
		return - 1;
	...
	boolean theCityIsInTheState(int cityIndx, int stateIndx) {
		...
		for (int i = 0; i < limit; i++) {
			if (cityIndx == constStruct.usCitiesInStates[k + i]) return true;
		}
		return false;
	}
	...
	public int findTheStateContainingTheCity(int cityIndex) {
		...
		for (int i = 0; i < constStruct.NUMBER_OF_US_STATES; i++) {
			...
			if (cityIndex == constStruct.usCitiesInStates[k + j]) return i;
		}
	...
	IOException {
		...
		// Query the employee names 
		if (tableNameIndex == constStruct.MEMBER_TABLE_INDEX) queryStmt = new String("SELECT login_id FROM " + constStruct.tableNames[tableNameIndex - 1]);
		else if (tableNameIndex == constStruct.EMPLOYER_TABLE_INDEX) queryStmt = new String("SELECT emp_id FROM " + constStruct.tableNames[tableNameIndex - 1]);
		else return false;
		Statement stmt = conn.createStatement();
		ResultSet rset = stmt.executeQuery(queryStmt);
